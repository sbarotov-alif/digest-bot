...
- Саммари групповых чатов в 20:00
...
# Каналы для мониторинга новостей
SOURCE_CHANNELS = [
    "spotuz",
    "RepostUZ",
    "davletovuz",
...
def summarize_chat(chat_name: str, messages: list) -> str:
    """Делает саммари переписки через Claude."""
    if not messages:
        return ""

    combined = "\n".join([f"- {m}" for m in messages[:150]])  # берём до 150 сообщений
    prompt = (
        f"Вот сообщения из рабочего чата «{chat_name}» за сегодня.\n"
        f"Сделай краткое резюме на русском: главные темы, решения, важные вопросы.\n"
        f"Пиши коротко и по делу, 5-8 пунктов максимум. Без лишних слов.\n\n"
        f"{combined}"
    )
...
async def fetch_group_messages(group_id: int, hours_back: int = 20) -> list:
    """Читает сообщения из группового чата за последние N часов."""
    client = TelegramClient("session_digest", API_ID, API_HASH)
    await client.start()
    ...
    try:
        ...
        for msg in history.messages:
            ...
            if len(text) < 10:  # пропускаем короткие сообщения типа "ок", "+"
                continue
            messages.append(text)

        logger.info(f"✅ Группа {group_id}: {len(messages)} сообщений")

    except Exception as e:
        logger.error(f"❌ Ошибка группы {group_id}: {e}")
...
async def send_daily_digest():
    ...
    if not posts:
        bot = telegram.Bot(token=BOT_TOKEN)
        await bot.send_message(chat_id=TARGET_CHANNEL, text="📭 За сегодня не найдено новостей по вашим темам.")
        return
    ...
    top_lines = ["🔥 *Топ-5 постов дня по просмотрам*\n"]
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    ...
    for part in [digest_text[i:i+max_len] for i in range(0, len(digest_text), max_len)]:
        await bot.send_message(chat_id=TARGET_CHANNEL, text=part, parse_mode="Markdown", disable_web_page_preview=True)
    ...
    logger.info("✅ Дайджест и топ-5 отправлены!")
...
async def send_group_summaries():
    ...
    header = f"💬 *Саммари чатов за {date_str}*"
    await bot.send_message(chat_id=TARGET_CHANNEL, text=header, parse_mode="Markdown")
    await asyncio.sleep(2)

    for group_id, group_name in GROUP_CHATS.items():
        logger.info(f"📖 Читаю группу: {group_name}...")
        messages = await fetch_group_messages(group_id, hours_back=20)

        if not messages:
            logger.info(f"⏭ {group_name}: сообщений нет")
            continue

        summary = summarize_chat(group_name, messages)
        if not summary:
            continue
...
if __name__ == "__main__":
    ...
    logger.info("📰 Дайджест + Топ-5 в 19:00 по Ташкенту")
    logger.info("💬 Саммари групп в 20:00 по Ташкенту")

    schedule.every(60).minutes.do(run_news)
    schedule.every().day.at("14:00").do(run_digest)   # 19:00 Ташкент
    schedule.every().day.at("15:00").do(run_summaries) # 20:00 Ташкент
